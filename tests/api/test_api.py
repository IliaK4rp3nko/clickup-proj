...
class TestPosts:
    ...
    def test_get_task_success(self, clickup_client, task_fixture):
        ...
        with allure.step("Проверка совпадения данных"):
            ...
            assert (
                task_data["status"]["status"] == task_fixture["status"]["status"]
            ), "Статус не совпадает"
    ...
    def test_update_task_success(self, clickup_client,
                                 task_fixture, updated_data):
        task_id = task_fixture["id"]
    ...
    def test_update_task_invalid_data(self, clickup_client,
                                      task_fixture, invalid_data):
        task_id = task_fixture["id"]
        ...
        with allure.step("Проверка сообщения об ошибке"):
            response_json = response.json()
            assert (
                "err" in response_json or "message" in response_json
            ), "Нет сообщения об ошибке"

    @allure.description("Удаление задачи")
    def test_delete_task_success(self, clickup_client,
